...
def make_withdraw(balance, password):
    ...
    def withdraw(amount, input):
        nonlocal balance, list_pass
        if len(list_pass) == 3:
            return "Your account is locked. Attempts: " + str(list_pass)
        elif input == password:
            if amount > balance:
...
class VendingMachine:
    ...
    def vend(self):
        ...
        elif self.amount_deposited < self.price:
            return 'You must deposit ${} more.'.format(self.price - self.amount_deposited)
        elif self.amount_deposited > self.price:
            self.stock -= 1
            balance = self.amount_deposited - self.price
            self.amount_deposited = 0
            return 'Here is your {} and ${} change.'.format(self.food, balance)
        else:
            self.stock -= 1
            self.amount_deposited = 0
            return 'Here is your {}.'.format(self.food)

    def restock(self, amount):
        self.stock += amount
        return 'Current {} stock: {}'.format(self.food, self.stock)
...
def remove_all(link , value):
    ...
    <0 1>
    """
    """look at bottom for Link classes"""

    if Link.empty is not link.rest and link is not Link.empty:
    ...
    """while link.rest is not Link.empty:
        if link.rest.first == value:
            link.rest = Link.empty.rest
        else:
            link = link.rest
        Link.empty = Link.empty.rest"""
...
def generate_paths(t, x):
    ...
    for br in t.branches:
        for rues in generate_paths(br, x):
            yield  [t.label] + rues
